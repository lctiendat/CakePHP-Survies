...
$i = 1;
$arrayNum = [];
foreach ($getAnswerBySurveyId as $item) {
    array_push($arrayNum, $i++);
}
$arrayNum = json_encode($arrayNum);
?>
<div class="container-fluid">
    <?php if (count($count) > 0) { ?>
        <div class="card shadow p-5 mb-4">
            <div class="row">
                <a href="/admin/survies"><button type="button" class="btn btn-primary float-left mb-3">Back <i class="fa fa-undo-alt"></i></button></a>

                <div class="col-md-12">
                    <?php foreach ($survey as $item) { ?>
                        <h5 class="font-weight-bold text-uppercase ">Thống kê dữ liệu kết quả khảo sát của <?= $item->question ?> </h5>
                    <?php } ?>
                </div>
            </div>
            <div class="row">
                <div class="col-md-3">
                    <ol class="mt-5">
                        <?php foreach ($getAnswerBySurveyId as $item) { ?>
                            <li class="font-weight-bold"><?= $item->name  ?> đã được bình chọn <?= $item->count ?> lần.</li>
                        <?php } ?>
                    </ol>
                </div>
                <div class="col-md-9">
